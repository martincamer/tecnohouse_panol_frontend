...
export const ModalCrearNuevoCajon = () => {
  const { register, handleSubmit } = useForm();
  const { createCajon } = useCajones();
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      await createCajon(cajonData);

      document.getElementById("modal_crear_nuevo_cajon").close();
    } catch (error) {
  ...
  return (
    <dialog id="modal_crear_nuevo_cajon" className="modal">
